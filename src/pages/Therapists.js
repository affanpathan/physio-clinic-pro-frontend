...
export default function Therapists({ clinicId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingId ? `${API_URL}/therapists/${editingId}` : `${API_URL}/therapists`;
      ...
      setMessage(editingId ? 'Therapist updated successfully' : 'Therapist created successfully');
      resetForm();
      fetchItems();
    } catch (err) {
      setMessage(err.message);
  ...
  const handleExport = async () => {
    setMessage('');
    try {
      await downloadCsvExport(`${API_URL}/therapists/export`, 'therapists_export.csv');
    } catch (err) {
      setMessage(err.message);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Delete this therapist record?')) return;
    try {
      const res = await fetch(`${API_URL}/therapists/${id}`, { method: 'DELETE' });
      const data = await parseJsonResponse(res);
  ...
  return (
    <div style={{ padding: 24 }}>
      <h2>Therapists</h2>
      <p style={{ color: '#666' }}>Manage therapist details and contact information.</p>
      ...
      <form onSubmit={handleSubmit} style={{ background: '#fff', padding: 16, borderRadius: 8, marginBottom: 20 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 12 }}>
          <input name="therapist_name" placeholder="Therapist Name" required value={form.therapist_name} onChange={handleChange} style={inputStyle} />
          <input name="phone" placeholder="Phone" value={form.phone} onChange={handleChange} style={inputStyle} />
          <textarea name="address" placeholder="Address" value={form.address} onChange={handleChange} style={{ ...inputStyle, minHeight: 42 }} />
        </div>
        ...
        <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
          <button type="submit" style={buttonStyle}>{editingId ? 'Update Therapist' : 'Create Therapist'}</button>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id}>
                  ...
                  <td style={tdStyle}>{item.therapist_name}</td>
